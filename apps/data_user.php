...
// ambil data
$user= "user";
 $data = query("SELECT * FROM login WHERE level = 'user' LIMIT $awaldata, $dataperhalaman");
 
 if( isset($_POST["cari"])){
  $data = cari_user($_POST["keyword"]);
...
<body>
<h1 class="text-black-30 mx-auto mt-2 mb-3">Data Pengguna</h1>
<div class="container">
  <div class="row">
    <div class="col-md">
...
<thead class="thead-light">
    ...
    <?php
        ...
        <div class="mt-2">
          ...
          <?php if ($aktif > 1 ) : ?>
          <a href="?page=data_user&&halaman=<?= $aktif - 1;  ?>" class="btn btn-primary">&laquo; Prev </a>
          <?php endif; ?>

          <?php for( $i=1; $i <= $jumlahhalaman; $i++ ) : ?>
            <?php if($i == $aktif ) : ?> 
            <a href="?page=data_user&&halaman=<?= $i; ?>" class="btn btn-primary"><?= $i; ?></a>
            <?php else: ?>
            <a href="?page=data_user&&halaman=<?= $i; ?>" class="btn btn-default"><?= $i; ?></a>
            <?php endif;?>
            <?php endfor; ?>

            <?php if ($aktif < $jumlahhalaman ) : ?>
            <a href="?page=data_user&&halaman=<?= $aktif + 1;  ?>" class="btn btn-primary">Next &raquo;</a>
          <?php endif; ?></div>
          </div>